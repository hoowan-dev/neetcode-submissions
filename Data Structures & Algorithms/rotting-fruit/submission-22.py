class Solution:
    def orangesRotting(self, grid: List[List[int]]) -> int:
        ...
        # collect rotten banana coords and put into stack
        ...
        # util for checking adjacent fresh banana
        ...
        # simulate rotting of adjacent bananas
        while freshCount > 0 and len(queue) > 0:
            generationCount = len(queue)
            for _ in range(generationCount):
                rottenBananaCoord = queue.popleft()
                x, y = rottenBananaCoord[0], rottenBananaCoord[1]
